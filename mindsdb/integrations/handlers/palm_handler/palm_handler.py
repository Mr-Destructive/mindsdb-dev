...
from mindsdb.integrations.handlers.palm_handler.helpers import retry_with_exponential_backoff, \
    truncate_msgs_for_token_limit
from mindsdb.integrations.utilities.handler_utils import get_api_key

CHAT_MODELS = ('chat-bison-001', 'text-bison-001', 'embedding-gecko-001',)
...
class PalmHandler(BaseMLEngine):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.default_model = 'text-bison-001'
        self.default_mode = 'default'  # can also be 'conversational' or 'conversational-full'
        self.supported_modes = ['default', 'conversational', 'conversational-full', 'embedding']
    ...
    def predict(self, df, args=None):
        ...
        # Chat or normal completion mode
            ...
            api_args = {
                'max_output_tokens': pred_args.get('max_output_tokens', args.get('max_output_tokens', self.default_max_output_tokens)),
    ...
    def _completion(self, model_name, prompts, api_key, api_args, args, df, parallel=True):
        ...
        There are a couple checks that should be done when calling palm's API:
        ...
        def _submit_embedding_completion(kwargs, prompts, api_args):
            ...
            before_palm_query(kwargs)
            resp = _tidy(palm.Embedding.create(**kwargs))
            _log_api_call(kwargs, resp)
            return resp
